...
import {
  ...
  fillPdcOtherDiagnosisHeaderMinimum,
  fillPdcOtherDiagnosisVisitForm,
} from '../../pages';
import { PDC_OTHER_DIAGNOSIS_FOLLOWUP_ENCOUNTER_TYPE_UUID } from '../../core/constants';

test.describe('PDC Other Diagnosis visit mastercard', () => {
  test.beforeEach(async ({ page, eligiblePdcPatient }) => {
    ...
    await fillPdcOtherDiagnosisHeaderMinimum(headerForm);
    await headerForm.save();
    await headerForm.expectSaveSuccess();
  });

  test('a follow-up visit encounter can be entered and is saved as PDC_OTHER_DIAGNOSIS_FOLLOWUP', async ({
    page,
    api,
    eligiblePdcPatient,
  }) => {
    const visitForm = new MastercardFormPage(page);
    await visitForm.enterNewFlowsheet('Other Diagnosis Visit');

    const appointmentDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    await fillPdcOtherDiagnosisVisitForm(visitForm, { appointmentDate });

    await visitForm.save();
    await visitForm.expectSaveSuccess();

    const res = await api.get(
      `encounter?patient=${eligiblePdcPatient.uuid}&encounterType=${PDC_OTHER_DIAGNOSIS_FOLLOWUP_ENCOUNTER_TYPE_UUID}&v=full`,
    );
    ...
    expect(obs.some((o) => /height.*75/i.test(o.display))).toBeTruthy();
    expect(obs.some((o) => /weight.*9/i.test(o.display))).toBeTruthy();
