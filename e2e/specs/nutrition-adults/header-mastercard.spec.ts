...
import { MastercardFormPage, NutritionAdultsMastercardGatePage, fillNutritionAdultsHeaderForm } from '../../pages';
import { NUTRITION_ADULTS_INITIAL_ENCOUNTER_TYPE_UUID } from '../../core/constants';

test('Nutrition Adults header mastercard saves the entered data as a NUTRITION_ADULTS_INITIAL encounter', async ({
  page,
  api,
  ...
  await fillNutritionAdultsHeaderForm(form, encounterDate);

  await form.save();
  await form.expectSaveSuccess();

  const res = await api.get(
    `encounter?patient=${eligibleNutritionPatient.uuid}&encounterType=${NUTRITION_ADULTS_INITIAL_ENCOUNTER_TYPE_UUID}&v=full`,
  );
