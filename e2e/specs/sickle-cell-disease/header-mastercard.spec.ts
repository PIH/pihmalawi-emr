...
test('Sickle Cell Disease header mastercard saves the entered data as a SICKLE_CELL_DISEASE_INITIAL encounter', async ({
  ...
  // its "Specify:" free text). The "In-Patient" checkbox's underlying concept
  // is "Patient is an inpatient at time of visit" (fixed by the
  // MLW-1854-sickle-cell-disease-concept-corrections work, which replaced the
  // previously mismatched "Patient transfer in" concept) — asserted here via
  // its own REST display string, not the on-screen "In-Patient" label, per
  // this suite's rule of asserting the real REST display string.
  expect(
    obs.some((o) => /pdc reasons for referral.*patient is an inpatient at time of visit/i.test(o.display)),
  ).toBeTruthy();
